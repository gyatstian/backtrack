...
3. A GPU scaler converts the frame to the active encoder's preferred format (BGRA for NVENC and AMF) and the capture thread pushes a `GpuFrame` containing `ID3D11Texture2D` into a lock-free single-producer/single-consumer queue.
...
- `IEncoder` abstracts the hardware encoder. `createEncoderForDevice` picks a backend from the D3D adapter vendor: NVIDIA -> `NvencEncoder` (NVENC), AMD (`VendorId 0x1002`) -> `AmfEncoder` (AMF). Other vendors default to NVENC.
- Both encoders share the D3D11 device with capture, so `ID3D11Texture2D` frames stay on the GPU. NVENC registers BGRA textures; AMF wraps BGRA via `AMFContext::CreateSurfaceFromDX11Native` and converts to NV12 inside the encoder.
- Each backend advertises `preferredInputFormat()` (BGRA for both). Capture may scale/letterbox; AMF avoids a separate NV12 video-processor pass.
- Keyframe detection, encoder-effective settings mapping, and per-backend capability queries follow the same pattern in both encoders. AMF inserts SPS/PPS (AVC) or VPS/SPS/PPS (HEVC) on IDRs; B-frames are disabled when zero-reorder-delay is required. Encoder faults trigger a full GPU pipeline recreate.
